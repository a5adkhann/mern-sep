...
const WhyFitness = () => {
  const features = [
    ...
    {
      icon: <HeartPulse className="w-10 h-10 text-cyan-400" />,
  ...
  return (
    <section className="relative text-white py-24 font-[Michroma] bg-gradient-to-b from-black via-[#0B1120] to-black overflow-hidden">
      {/* Animated Orbs */}
      <motion.div
        className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(56,189,248,0.1),transparent_60%)]"
        animate={{ opacity: [0.6, 0.8, 0.6] }}
        transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute inset-0 bg-[radial-gradient(circle_at_80%_80%,rgba(14,165,233,0.1),transparent_70%)]"
      ...
      <div className="relative z-10 text-center max-w-6xl mx-auto px-4 sm:px-6">
        <motion.h2
          ...
          className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-4 sm:mb-6 bg-gradient-to-r from-blue-400 via-cyan-400 to-indigo-500 text-transparent bg-clip-text px-2"
        >
          Why Fitness Matters
        </motion.h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 md:gap-10">
          {features.map((item, index) => (
            <motion.div
              ...
              className="bg-white/10 backdrop-blur-md border border-blue-400/20 rounded-2xl p-8 hover:bg-white/20 hover:scale-105 transition-all duration-300 w-full"
            >
              <div className="flex flex-col items-center text-center space-y-5">
                <div className="p-5 bg-gradient-to-br from-blue-400/10 to-cyan-400/10 rounded-full shadow-md shadow-blue-500/10">
                  {item.icon}
                </div>
